...
(см. раздел «Порядок — Message Passing (happens-before)» в корневом README).
...
которое вместе с **program order** образует **happens-before** (см. разделы 15–16 корневого README).
...
──────────────────                       ────────────────
ptr::write(slot, value)   (po)
       ↓                                 tail.load(Acquire)
tail.store(Release)  ───sw──────────────→      ↓       (po)
                                         ptr::read(slot)
                                               ↓       (po)
head.load(Acquire)   ←──sw──────────────  head.store(Release)
       ↓             (po)
ptr::write(slot, next_value)
```
...
согласованно (см. раздел 19 корневого README):
...
|---|---|---|---|
| `tail` | Производитель | `tail.load(Relaxed)` | `tail.load(Acquire)` у потребителя |
| `head` | Потребитель | `head.load(Relaxed)` | `head.load(Acquire)` у производителя |
