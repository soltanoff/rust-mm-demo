...
От противного: пусть два потока одновременно оказались в критической секции.
Посмотрим на тот поток, который при этом сделал запись в victim_ последним. Пусть это поток под номером
...
2. T0: victim_ = 0
3. T1: victim_ = 1
4. T1: want_[0] == true, victim_ == 1
```

## 2. Точка с запятой

Пусть вы написали в программе:
...
Компилятор может очень вольно трактовать этот код:
...
Source: https://godbolt.org/, x86-64, gcc 8.2, -02

Обратите внимание! Компилятор переставил инструкции, воспользовавшись регистрами. Компилятор не знает, что компилируемый
код будет работать в многопоточном окружении. Он заботится лишь о том, чтобы не сломать **наблюдаемое** поведение
**однопоточной** программы (уважает data и control dependencies).
...
Аналог атомиков в Java - **volatile** (при этом в С++ y volatile **нет** такой семантики).
...
![drf-reorderings.png](assets/drf-reorderings.png)
